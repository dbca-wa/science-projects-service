...
class AgencyService:
    ...
    @staticmethod
    def delete_affiliation_simple(pk, user):
        """Delete affiliation (simple version without cleanup)"""
        affiliation = AgencyService.get_affiliation(pk)
    ...
    def delete_affiliation(pk: int, user) -> dict:
        ...
        for ext in external_projects:
            if ext.collaboration_with:
                affiliations = [
                    a.strip() for a in ext.collaboration_with.split("; ") if a.strip()
                ]
        ...
        for student in student_projects:
            if student.organisation:
                affiliations = [
                    a.strip() for a in student.organisation.split("; ") if a.strip()
                ]
                affiliations = [a for a in affiliations if a != affiliation.name]
    ...
    def clean_orphaned_affiliations(user) -> dict:
        ...
        for affiliation in all_affiliations:
            ...
            user_refs = UserWork.objects.filter(affiliation=affiliation.pk).count()
        ...
        for aff in orphaned_affiliations:
            settings.LOGGER.info(
                f"Deleting orphaned affiliation: {aff.name} (pk={aff.pk})"
            )
            aff.delete()
