...
from documents.models import ProjectDocument, ProgressReport
...
class TestConceptPlans:
    """Tests for concept plan list and create endpoints"""

    def test_list_concept_plans_authenticated(self, api_client, user, project_document, db):
    ...
    @patch('documents.services.concept_plan_service.ConceptPlanService.create_concept_plan')
    def test_create_concept_plan(self, mock_create, api_client, user, project_with_lead, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('conceptplans'), data, format='json')
...
class TestConceptPlanDetail:
    """Tests for concept plan detail endpoints"""
    ...
    def test_get_concept_plan(self, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('conceptplans', project_document.id))
    ...
    def test_delete_concept_plan(self, mock_delete, api_client, user, project_document, db):
        """Test deleting a concept plan"""
        # Arrange
        api_client.force_authenticate(user=user)
        
        # Act
        response = api_client.delete(documents_urls.path('conceptplans', project_document.id))
...
class TestRequestApproval:
    ...
    def test_request_approval_success(self, mock_get, mock_request, api_client, user, project_document, db):
        ...
        response = api_client.post(documents_urls.path('actions', 'request_approval', project_document.id))
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_request_approval_unauthenticated(self, mock_get, api_client, project_document, db):
    ...
    @patch('documents.services.approval_service.ApprovalService.request_approval')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_request_approval_service_error(self, mock_get, mock_request, api_client, user, project_document, db):
...
class TestApproveStageOne:
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_one_success(self, mock_get, mock_approve, api_client, user, project_document, db):
        ...
        response = api_client.post(documents_urls.path('actions', 'approve', 'stage1', project_document.id))
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_one_unauthenticated(self, mock_get, api_client, project_document, db):
    ...
    @patch('documents.services.approval_service.ApprovalService.approve_stage_one')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_one_permission_denied(self, mock_get, mock_approve, api_client, user, project_document, db):
...
class TestApproveStageTwo:
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_two_success(self, mock_get, mock_approve, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('actions', 'approve', 'stage2', project_document.id))
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_two_unauthenticated(self, mock_get, api_client, project_document, db):
    ...
    @patch('documents.services.approval_service.ApprovalService.approve_stage_two')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_two_stage_one_incomplete(self, mock_get, mock_approve, api_client, user, project_document, db):
...
class TestApproveStageThree:
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_three_success(self, mock_get, mock_approve, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('actions', 'approve', 'stage3', project_document.id))
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_three_unauthenticated(self, mock_get, api_client, project_document, db):
    ...
    @patch('documents.services.approval_service.ApprovalService.approve_stage_three')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_approve_stage_three_permission_denied(self, mock_get, mock_approve, api_client, user, project_document, db):
...
class TestSendBack:
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_send_back_success(self, mock_get, mock_send_back, api_client, user, project_document, db):
        ...
        data = {
            'reason': 'Needs more detail in methodology section'
        }
        ...
        assert response.status_code in [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND]
        if response.status_code == status.HTTP_200_OK:
            mock_send_back.assert_called_once_with(project_document, user, 'Needs more detail in methodology section')
            assert 'id' in response.data

    @patch('documents.services.approval_service.ApprovalService.send_back')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_send_back_no_reason(self, mock_get, mock_send_back, api_client, user, project_document, db):
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_send_back_unauthenticated(self, mock_get, api_client, project_document, db):
        """Test sending back without authentication"""
        # Arrange
        mock_get.return_value = project_document
        
        # Act
        response = api_client.post(documents_urls.path('actions', 'send_back', project_document.id))
    ...
    @patch('documents.services.approval_service.ApprovalService.send_back')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_send_back_permission_denied(self, mock_get, mock_send_back, api_client, user, project_document, db):
...
class TestRecallDocument:
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_recall_success(self, mock_get, mock_recall, api_client, user, project_document, db):
        ...
        data = {
            'reason': 'Need to make significant changes'
        }
        ...
        assert response.status_code in [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND]
        if response.status_code == status.HTTP_200_OK:
            mock_recall.assert_called_once_with(project_document, user, 'Need to make significant changes')
            assert 'id' in response.data

    @patch('documents.services.approval_service.ApprovalService.recall')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_recall_no_reason(self, mock_get, mock_recall, api_client, user, project_document, db):
    ...
    def test_recall_unauthenticated(self, mock_get, api_client, project_document, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('actions', 'recall', project_document.id))
    ...
    @patch('documents.services.approval_service.ApprovalService.recall')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_recall_permission_denied(self, mock_get, mock_recall, api_client, user, project_document, db):
...
class TestBatchApprove:
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_batch_approve_success(self, mock_get, mock_batch, api_client, user, project_document, db):
        ...
        data = {
            'document_ids': [project_document.id],
            'stage': 1
        }
        
        # Act
        response = api_client.post(documents_urls.path('batchapprove'), data, format='json')
    ...
    @patch('documents.services.approval_service.ApprovalService.batch_approve')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_batch_approve_multiple_documents(self, mock_get, mock_batch, api_client, user, project_document, db):
        """Test batch approving multiple documents at once"""
        # Arrange
        api_client.force_authenticate(user=user)
        doc2 = ProjectDocumentFactory(project=project_document.project, kind='projectplan')
        mock_get.side_effect = [project_document, doc2]
        mock_batch.return_value = {'approved': 2, 'failed': 0}
        
        data = {
            'document_ids': [project_document.id, doc2.id],
            'stage': 2
        }
    ...
    @patch('documents.services.approval_service.ApprovalService.batch_approve')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_batch_approve_stage_1(self, mock_get, mock_batch, api_client, user, project_document, db):
    ...
    @patch('documents.services.approval_service.ApprovalService.batch_approve')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_batch_approve_stage_2(self, mock_get, mock_batch, api_client, user, project_document, db):
    ...
    @patch('documents.services.approval_service.ApprovalService.batch_approve')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_batch_approve_stage_3(self, mock_get, mock_batch, api_client, user, project_document, db):
    ...
    @patch('documents.services.approval_service.ApprovalService.batch_approve')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_batch_approve_partial_failure(self, mock_get, mock_batch, api_client, user, project_document, db):
        """Test batch approve with some failures"""
        # Arrange
        api_client.force_authenticate(user=user)
        doc2 = ProjectDocumentFactory(project=project_document.project, kind='projectplan')
        mock_get.side_effect = [project_document, doc2]
        mock_batch.return_value = {'approved': 1, 'failed': 1, 'errors': ['Document 2 failed']}
...
class TestDownloadProjectDocument:
    ...
    def test_download_document_generates_pdf(self, mock_generate, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('downloadProjectDocument', project_document.id))
    ...
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_download_document_with_existing_pdf(self, mock_get_doc, api_client, user, project_document, db):
...
class TestBeginProjectDocGeneration:
    ...
    def test_begin_pdf_generation(self, mock_complete, mock_start, mock_generate, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('generate_project_document', project_document.id))
    ...
    @patch('documents.services.pdf_service.PDFService.mark_pdf_generation_started')
    @patch('documents.services.pdf_service.PDFService.mark_pdf_generation_complete')
    def test_begin_pdf_generation_with_exception(self, mock_complete, mock_start, mock_generate, api_client, user, project_document, db):
    ...
    def test_begin_pdf_generation_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('generate_project_document', 99999))
        
        # Assert
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_begin_pdf_generation_unauthenticated(self, api_client, project_document, db):
...
class TestCancelProjectDocGeneration:
    ...
    def test_cancel_pdf_generation(self, mock_cancel, api_client, user, project_document, db):
        """Test cancelling PDF generation - covers lines 82-88"""
        # Arrange
        api_client.force_authenticate(user=user)
        
        # Act
        response = api_client.post(documents_urls.path('cancel_doc_gen', project_document.id))
    ...
    def test_cancel_pdf_generation_unauthenticated(self, api_client, project_document, db):
...
class TestDownloadAnnualReport:
    ...
    def test_download_annual_report(self, mock_generate, api_client, user, annual_report, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('reports/download', annual_report.id))
        
        # Assert
        assert response.status_code in [status.HTTP_200_OK, status.HTTP_400_BAD_REQUEST, status.HTTP_404_NOT_FOUND]

    def test_download_annual_report_with_existing_pdf(self, api_client, user, annual_report, db):
    ...
    def test_download_annual_report_unauthenticated(self, api_client, annual_report, db):
...
class TestBeginAnnualReportDocGeneration:
    ...
    @patch('documents.services.pdf_service.PDFService.mark_pdf_generation_started')
    @patch('documents.services.pdf_service.PDFService.mark_pdf_generation_complete')
    def test_begin_annual_report_generation(self, mock_complete, mock_start, mock_generate, api_client, user, annual_report, db):
    ...
    def test_begin_annual_report_generation_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('reports', 99999, 'generate_pdf'))
        
        # Assert
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_begin_annual_report_generation_unauthenticated(self, api_client, annual_report, db):
...
class TestCancelReportDocGeneration:
    ...
    @patch('documents.services.pdf_service.PDFService.cancel_pdf_generation')
    def test_cancel_report_generation(self, mock_cancel, api_client, user, annual_report, db):
        """Test cancelling annual report PDF generation - covers lines 165-171"""
        # Arrange
        api_client.force_authenticate(user=user)
        
        # Act
        response = api_client.post(documents_urls.path('reports', annual_report.id, 'cancel_doc_gen'))
    ...
    def test_cancel_report_generation_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('reports', 99999, 'cancel_doc_gen'))
        
        # Assert
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_cancel_report_generation_unauthenticated(self, api_client, annual_report, db):
...
class TestNewCycleOpen:
    ...
    def test_new_cycle_open_superuser(self, mock_send_email, api_client, superuser, annual_report, project_with_lead, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('opennewcycle'), data, format='json')
    ...
    def test_new_cycle_open_unauthenticated(self, api_client, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('opennewcycle'), {}, format='json')
        
        # Assert
        assert response.status_code in [status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN]

    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_new_cycle_open_with_emails(self, mock_send_email, api_client, superuser, annual_report, project_with_lead, business_area_with_leader, db):
...
class TestSendBumpEmails:
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_bump_emails_admin(self, mock_send_email, api_client, admin_user, project_with_lead, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('sendbumpemails'), data, format='json')
    ...
    def test_send_bump_emails_unauthenticated(self, api_client, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('sendbumpemails'), {}, format='json')
        
        # Assert
        assert response.status_code in [status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN]

    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_bump_emails_multiple_documents(self, mock_send_email, api_client, admin_user, project_with_lead, user_factory, db):
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_bump_emails_inactive_user(self, mock_send_email, api_client, admin_user, project_with_lead, user_factory, db):
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_bump_emails_user_not_found(self, mock_send_email, api_client, admin_user, project_with_lead, db):
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_bump_emails_email_error(self, mock_send_email, api_client, admin_user, project_with_lead, db):
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_bump_emails_different_document_kinds(self, mock_send_email, api_client, admin_user, project_with_lead, db):
        """Test sending bump emails for different document kinds"""
        # Arrange
        api_client.force_authenticate(user=admin_user)
        document_kinds = ['concept', 'projectplan', 'progressreport', 'studentreport', 'projectclosure']
...
class TestUserPublications:
    ...
    def test_get_user_publications_authenticated(self, mock_get, api_client, user, staff_profile, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('publications', staff_profile.employee_id))
    ...
    @patch('documents.views.notifications.requests.get')
    def test_get_user_publications_unauthenticated(self, mock_get, api_client, staff_profile, db):
    ...
    @patch('documents.views.notifications.requests.get')
    def test_get_user_publications_api_error(self, mock_get, api_client, user, staff_profile, db):
    ...
    @patch('documents.views.notifications.requests.get')
    def test_get_user_publications_cached(self, mock_get, api_client, user, staff_profile, db):
    ...
    @patch('documents.views.notifications.requests.get')
    def test_get_user_publications_with_custom_publications(self, mock_get, api_client, user, staff_profile, db):
    ...
    @patch('documents.views.notifications.requests.get')
    def test_get_user_publications_no_staff_profile(self, mock_get, api_client, user, db):
    ...
    def test_get_user_publications_missing_api_url(self, mock_settings, api_client, user, staff_profile, db):
    ...
    @patch('documents.views.notifications.settings')
    def test_get_user_publications_missing_token(self, mock_settings, api_client, user, staff_profile, db):
...
class TestSendMentionNotification:
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_mention_notification(self, mock_send_email, api_client, user, project_document, project_with_lead, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('notifications', 'mentions'), data, format='json')
        
        # Assert
        assert response.status_code == status.HTTP_200_OK
        assert 'message' in response.data

    def test_send_mention_notification_no_mentioned_users(self, api_client, user, project_document, project_with_lead, db):
    ...
    def test_send_mention_notification_unauthenticated(self, api_client, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('notifications', 'mentions'), {}, format='json')
        
        # Assert
        assert response.status_code in [status.HTTP_401_UNAUTHORIZED, status.HTTP_403_FORBIDDEN]

    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_mention_notification_with_html_content(self, mock_send_email, api_client, user, project_document, project_with_lead, db):
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_mention_notification_inactive_user(self, mock_send_email, api_client, user, project_document, project_with_lead, user_factory, db):
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_mention_notification_non_dbca_email(self, mock_send_email, api_client, user, project_document, project_with_lead, user_factory, db):
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_mention_notification_user_not_found(self, mock_send_email, api_client, user, project_document, project_with_lead, db):
    ...
    @patch('documents.views.notifications.send_email_with_embedded_image')
    def test_send_mention_notification_email_error(self, mock_send_email, api_client, user, project_document, project_with_lead, db):
        ...
        # Assert
        assert response.status_code == status.HTTP_200_OK  # Still returns 200 even if email fails
...
class TestProjectDocsPendingMyActionAllStages:
    ...
    def test_get_pending_documents_authenticated(self, api_client, user_with_work, project_with_lead, project_document, db):
        """Test getting pending documents as authenticated user"""
        # Arrange
        api_client.force_authenticate(user=user_with_work)
        
        # Act
        response = api_client.get(documents_urls.path('projectdocuments', 'pendingmyaction'))
    ...
    def test_get_pending_documents_as_project_lead(self, api_client, user_with_work, project_with_lead, db):
    ...
    def test_get_pending_documents_as_ba_leader(self, api_client, user_with_work, project_with_ba_lead, db):
    ...
    def test_get_pending_documents_as_directorate(self, api_client, user_with_work, project_with_lead, db):
...
class TestGetPreviousReportsData:
    ...
    def test_get_previous_progress_report_data(self, api_client, user, project_with_lead, annual_report, progress_report_with_details, db):
        """Test getting previous progress report data"""
        # Arrange
        api_client.force_authenticate(user=user)
        
        # Create an older progress report (progress_report_with_details is the newer one)
        from documents.models import ProgressReport, AnnualReport
        ...
        pr1 = ProgressReport.objects.create(
            document=old_doc,
            project=project_with_lead,
            report=old_report,
            year=2022,
        ...
        # Act
        response = api_client.post(documents_urls.path('get_previous_reports_data'), data, format='json')
        
        # Assert
        assert response.status_code == status.HTTP_200_OK
        assert response.data == '<p>Old context</p>'

    def test_get_previous_student_report_data(self, api_client, user, project_with_lead, annual_report, student_report_with_details, db):
        """Test getting previous student report data"""
        # Arrange
        api_client.force_authenticate(user=user)
        
        # Create an older student report (student_report_with_details is the newer one)
        from documents.models import StudentReport, AnnualReport
        from datetime import datetime
        old_report = AnnualReport.objects.create(
            year=2022,
            is_published=False,
        ...
        sr1 = StudentReport.objects.create(
            document=old_doc,
            project=student_project,
            report=old_report,
            year=2022,
    ...
    def test_get_previous_reports_data_invalid_kind(self, api_client, user, project_with_lead, db):
    ...
    def test_get_previous_reports_data_insufficient_documents(self, api_client, user, project_with_lead, progress_report_with_details, db):
    ...
    def test_get_previous_reports_data_invalid_section(self, api_client, user, project_with_lead, annual_report, progress_report_with_details, db):
    ...
    def test_get_previous_reports_data_unauthenticated(self, api_client, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('get_previous_reports_data'), {}, format='json')
...
class TestReopenProject:
    ...
    @patch('documents.services.notification_service.NotificationService.notify_project_reopened')
    def test_reopen_project_with_closure_document(self, mock_notify, api_client, user, project_with_lead, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('projectclosures', 'reopen', project_with_lead.id))
    ...
    @patch('documents.services.notification_service.NotificationService.notify_project_reopened')
    def test_reopen_project_without_closure_document(self, mock_notify, api_client, user, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('projectclosures', 'reopen', project.id))
...
class TestBatchApproveOld:
    ...
    def test_batch_approve_old_as_superuser(self, api_client, superuser, project_with_lead, annual_report, progress_report_with_details, db):
...
class TestFinalDocApproval:
    ...
    def test_final_approval_grant(self, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('actions', 'finalApproval'), data, format='json')
    ...
    def test_final_approval_unauthenticated(self, api_client, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('actions', 'finalApproval'), {}, format='json')
...
class TestProjectPlans:
    ...
    def test_create_project_plan_valid_data(self, api_client, user, project_with_lead, db):
        """Test creating project plan with valid data - covers lines 48-52"""
        # Arrange
        api_client.force_authenticate(user=user)
        
        # Mock both serializers to test the successful creation path
        with patch('documents.views.project_plan.ProjectPlanSerializer') as mock_serializer_class, \
             patch('documents.views.project_plan.TinyProjectPlanSerializer') as mock_tiny_serializer_class:
            ...
            mock_tiny_serializer.data = {'id': project_plan.id, 'background': '<p>Test</p>'}
            ...
            # Act
            response = api_client.post(documents_urls.path('projectplans'), data, format='json')
...
class TestProjectPlanDetail:
    ...
    def test_update_project_plan_with_endorsement_data_management(self, api_client, user, db):
    ...
    def test_update_project_plan_auto_clear_animal_endorsement(self, api_client, user, db):
    ...
    def test_update_project_plan_auto_clear_plant_endorsement(self, api_client, user, db):
    ...
    def test_update_project_plan_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.put(
            documents_urls.path('projectplans', 99999),
            data,
            format='json'
        )
    ...
    def test_delete_project_plan(self, api_client, user, db):
        ...
        # Act
        response = api_client.delete(documents_urls.path('projectplans', project_plan.id))
    ...
    def test_update_project_plan_invalid_data(self, api_client, user, db):
        ...
        with patch('documents.views.project_plan.ProjectPlanSerializer') as mock_serializer_class:
            ...
            data = {
                'background': '',  # Invalid empty value
            }
...
class TestEndorsementDetail:
    ...
    def test_get_endorsement(self, api_client, user, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('projectplans', 'endorsements', endorsement.id))
    ...
    def test_get_endorsement_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('projectplans', 'endorsements', 99999))
    ...
    def test_get_endorsement_unauthenticated(self, api_client, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('projectplans', 'endorsements', 1))
    ...
    def test_update_endorsement_invalid_data(self, api_client, user, db):
        ...
        # Mock serializer to return invalid
        with patch('documents.views.endorsement.EndorsementSerializer') as mock_serializer_class:
            mock_serializer = Mock()
            mock_serializer.is_valid.return_value = False
...
class TestEndorsementsPendingMyAction:
    ...
    def test_get_pending_endorsements_as_aec(self, api_client, user, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('endorsements', 'pendingmyaction'))
...
class TestDeleteAECEndorsement:
    ...
    def test_delete_aec_endorsement_with_pdf(self, api_client, user, db):
        ...
        # Act
        response = api_client.post(
            documents_urls.path('project_plans', project_plan.id, 'delete_aec_endorsement_pdf')
        )
...
class TestCustomPublications:
    ...
    def test_list_custom_publications_authenticated(self, api_client, user, staff_profile, db):
    ...
    def test_create_custom_publication_valid_data(self, api_client, user, staff_profile, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('custompublications'), data, format='json')
...
class TestCustomPublicationDetail:
    ...
    def test_update_custom_publication_valid_data(self, api_client, user, staff_profile, db):
        ...
        # Act
        response = api_client.put(
            documents_urls.path('custompublications', pub.id),
            data,
            format='json'
        )
    ...
    def test_update_custom_publication_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.put(
            documents_urls.path('custompublications', 99999),
            data,
            format='json'
        )
    ...
    def test_update_custom_publication_invalid_data(self, api_client, user, staff_profile, db):
...
class TestProgressReports:
    ...
    def test_list_progress_reports_authenticated(self, api_client, user, progress_report, db):
    ...
    def test_create_progress_report_valid_data(self, api_client, user, project_document, annual_report, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('progressreports'), data, format='json')
...
class TestProgressReportDetail:
    ...
    def test_get_progress_report(self, api_client, user, progress_report, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('progressreports', progress_report.id))
    ...
    def test_update_progress_report_valid_data(self, api_client, user, progress_report, db):
    ...
    def test_update_progress_report_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.put(
            documents_urls.path('progressreports', 99999),
            data,
            format='json'
        )
    ...
    def test_update_progress_report_invalid_data(self, api_client, user, progress_report, db):
    ...
    def test_delete_progress_report(self, api_client, user, progress_report, db):
        ...
        # Act
        response = api_client.delete(documents_urls.path('progressreports', progress_report.id))
...
class TestUpdateProgressReport:
    ...
    def test_update_progress_report_section(self, api_client, user, progress_report, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('progress_reports/update'), data, format='json')
        
        # Assert
        assert response.status_code in [status.HTTP_202_ACCEPTED, status.HTTP_400_BAD_REQUEST, status.HTTP_404_NOT_FOUND]
    ...
    def test_update_progress_report_section_invalid_data(self, api_client, user, progress_report, db):
...
class TestProgressReportByYear:
    ...
    def test_get_progress_report_by_year(self, api_client, user, progress_report, db):
        ...
        response = api_client.get(
            documents_urls.path('progressreports', progress_report.document.project.id, progress_report.year)
        )
        
        # Assert
        assert response.status_code in [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND]

    def test_get_progress_report_by_year_not_found(self, api_client, user, project_with_lead, db):
    ...
    def test_get_progress_report_by_year_unauthenticated(self, api_client, progress_report, db):
...
class TestStudentReports:
    ...
    def test_list_student_reports_authenticated(self, api_client, user, student_report, db):
    ...
    def test_create_student_report_valid(self, api_client, user, student_report, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('studentreports'), data, format='json')
...
class TestStudentReportDetail:
    ...
    def test_get_student_report(self, api_client, user, student_report, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('studentreports', student_report.id))
    ...
    def test_update_student_report_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.put(
            documents_urls.path('studentreports', 99999),
            data,
            format='json'
        )
    ...
    def test_delete_student_report(self, api_client, user, student_report, db):
        ...
        # Act
        response = api_client.delete(documents_urls.path('studentreports', student_report.id))
...
class TestUpdateStudentReport:
    ...
    def test_update_student_report_content_valid(self, api_client, user, student_report, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('student_reports/update_progress'), data, format='json')
    ...
    def test_update_student_report_content_invalid(self, api_client, user, student_report, db):
        """Test updating student report with invalid data"""
        # Arrange
        api_client.force_authenticate(user=user)
...
class TestStudentReportByYear:
    ...
    def test_get_student_report_by_year(self, api_client, user, student_report, db):
        ...
        # Act
        response = api_client.get(
            documents_urls.path('studentreports', student_report.project.id, student_report.year)
        )
        
        # Assert
        assert response.status_code in [status.HTTP_200_OK, status.HTTP_404_NOT_FOUND]

    def test_get_student_report_by_year_not_found(self, api_client, user, project_with_lead, db):
    ...
    def test_get_student_report_by_year_unauthenticated(self, api_client, student_report, db):
...
class TestProjectClosures:
    ...
    def test_list_project_closures_authenticated(self, api_client, user, project_closure, db):
    ...
    def test_create_project_closure_valid(self, api_client, user, project_closure, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('projectclosures'), data, format='json')
...
class TestProjectClosureDetail:
    ...
    def test_get_project_closure(self, api_client, user, project_closure, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('projectclosures', project_closure.id))
    ...
    def test_update_project_closure_not_found(self, api_client, user, db):
        ...
        # Act
        response = api_client.put(
            documents_urls.path('projectclosures', 99999),
            data,
            format='json'
        )
    ...
    def test_update_project_closure_invalid(self, api_client, user, project_closure, db):
    ...
    def test_delete_project_closure(self, api_client, user, project_closure, db):
        ...
        # Act
        response = api_client.delete(documents_urls.path('projectclosures', project_closure.id))
...
class TestReportDetail:
    ...
    def test_update_report_valid(self, api_client, user, annual_report, db):
        ...
        # Act
        response = api_client.put(
            documents_urls.path('reports', annual_report.id),
            data,
            format='json'
        )
...
class TestGetAvailableReportYears:
    ...
    def test_get_available_years_for_student_report(self, api_client, user, annual_report, project_with_lead, db):
        ...
        # Act
        response = api_client.get(
            documents_urls.path('reports/availableyears', project_with_lead.id, 'studentreport')
        )
    ...
    def test_get_available_years_for_progress_report(self, api_client, user, annual_report, project_with_lead, db):
        ...
        # Act
        response = api_client.get(
            documents_urls.path('reports/availableyears', project_with_lead.id, 'progressreport')
        )
...
class TestLatestYearsReports:
    ...
    def test_get_latest_years_progress_reports(self, api_client, user, annual_report, db):
    ...
    def test_get_latest_years_student_reports(self, api_client, user, annual_report, db):
    ...
    def test_get_latest_years_inactive_reports(self, api_client, user, annual_report, db):
...
class TestConceptPlans:
    ...
    def test_list_concept_plans_authenticated(self, api_client, project_lead, concept_plan_with_details, db):
        """Test listing concept plans as authenticated user"""
        # Arrange
    ...
    @patch('documents.services.concept_plan_service.ConceptPlanService.create_concept_plan')
    def test_create_concept_plan_valid(self, mock_create, api_client, project_lead, concept_plan_with_details, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('conceptplans'), data=data, format='json')
    ...
    @patch('documents.services.concept_plan_service.ConceptPlanService.create_concept_plan')
    def test_create_concept_plan_no_details(self, mock_create, api_client, project_lead, concept_plan, db):
...
class TestConceptPlanDetail:
    ...
    def test_get_concept_plan_authenticated(self, api_client, project_lead, concept_plan_with_details, db):
        """Test getting concept plan as authenticated user"""
        # Arrange
        api_client.force_authenticate(user=project_lead)
        
        # Act
        response = api_client.get(documents_urls.path('conceptplans', concept_plan_with_details.document.id))
    ...
    def test_get_concept_plan_unauthenticated(self, api_client, concept_plan_with_details, db):
        """Test getting concept plan without authentication"""
        # Act
        response = api_client.get(documents_urls.path('conceptplans', concept_plan_with_details.document.id))
    ...
    @patch('documents.services.concept_plan_service.ConceptPlanService.get_concept_plan_data')
    def test_get_concept_plan_no_details(self, mock_get_data, api_client, project_lead, concept_plan, db):
    ...
    @patch('documents.services.concept_plan_service.ConceptPlanService.update_concept_plan')
    def test_update_concept_plan_valid(self, mock_update, api_client, project_lead, concept_plan_with_details, db):
    ...
    def test_update_concept_plan_invalid_data(self, api_client, project_lead, concept_plan_with_details, db):
    ...
    @patch('documents.services.concept_plan_service.ConceptPlanService.update_concept_plan')
    @patch('documents.services.concept_plan_service.ConceptPlanService.get_concept_plan_data')
    def test_update_concept_plan_no_details(self, mock_get_data, mock_update, api_client, project_lead, concept_plan, db):
    ...
    @patch('documents.services.document_service.DocumentService.delete_document')
    def test_delete_concept_plan_authenticated(self, mock_delete, api_client, project_lead, concept_plan_with_details, db):
        """Test deleting concept plan as authenticated user"""
        # Arrange
        api_client.force_authenticate(user=project_lead)
        
        # Act
        response = api_client.delete(documents_urls.path('conceptplans', concept_plan_with_details.document.id))
        
        # Assert
        assert response.status_code == status.HTTP_204_NO_CONTENT
        mock_delete.assert_called_once_with(concept_plan_with_details.document.id, project_lead)

    def test_delete_concept_plan_unauthenticated(self, api_client, concept_plan_with_details, db):
...
class TestGetConceptPlanData:
    ...
    def test_get_concept_plan_data_authenticated(self, api_client, project_lead, concept_plan_with_details, db):
        ...
        # Act
        response = api_client.post(
            documents_urls.path('conceptplans', concept_plan_with_details.id, 'get_concept_plan_data')
        )
    ...
    def test_get_concept_plan_data_unauthenticated(self, api_client, concept_plan_with_details, db):
    ...
    def test_get_concept_plan_data_with_team(self, api_client, project_lead, concept_plan_with_details, user_factory, db):
    ...
    def test_get_concept_plan_data_with_image(self, api_client, project_lead, concept_plan_with_details, db):
    ...
    def test_get_concept_plan_data_without_image(self, api_client, project_lead, concept_plan_with_details, db):
    ...
    def test_get_concept_plan_data_approval_statuses(self, api_client, project_lead, concept_plan_with_details, db):
    ...
    def test_get_concept_plan_data_formatted_tables(self, api_client, project_lead, concept_plan_with_details, db):
...
class TestProjectDocuments:
    ...
    def test_list_documents_with_pagination(self, api_client, user, project_with_lead, db):
    ...
    def test_list_documents_with_kind_filter(self, api_client, user, project_with_lead, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('projectdocuments'), {'kind': 'concept'})
        
        # Assert
        assert response.status_code == status.HTTP_200_OK
        assert 'documents' in response.data

    def test_list_documents_with_status_filter(self, api_client, user, project_with_lead, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('projectdocuments'), {'status': 'new'})
        
        # Assert
        assert response.status_code == status.HTTP_200_OK
        assert 'documents' in response.data

    def test_list_documents_with_search_term(self, api_client, user, project_with_lead, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('projectdocuments'), {'searchTerm': 'Unique'})
    ...
    def test_create_document_valid(self, api_client, user, project_with_lead, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('projectdocuments'), data, format='json')
...
class TestProjectDocumentDetail:
    ...
    def test_get_document_authenticated(self, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.get(documents_urls.path('projectdocuments', project_document.id))
    ...
    def test_delete_document_authenticated(self, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.delete(documents_urls.path('projectdocuments', document_id))
    ...
    def test_delete_document_unauthenticated(self, api_client, project_document, db):
        ...
        # Act
        response = api_client.delete(documents_urls.path('projectdocuments', project_document.id))
...
class TestProjectDocsPendingMyAction:
    ...
    def test_get_pending_documents_authenticated(self, api_client, project_lead, project_with_lead, db):
        """Test getting pending documents as authenticated user"""
        # Arrange
        api_client.force_authenticate(user=project_lead)
    ...
    def test_get_pending_documents_with_stage_filter(self, api_client, project_lead, project_with_lead, db):
    ...
    def test_get_pending_documents_without_stage_filter(self, api_client, project_lead, project_with_lead, db):
    ...
    def test_get_pending_documents_stage_2(self, api_client, ba_lead, project_with_ba_lead, db):
...
class TestProjectDocsPendingMyActionUnused:
    ...
    def test_unused_view_get_with_stage(self, api_client, project_lead, project_with_lead, db):
    ...
    def test_unused_view_get_without_stage(self, api_client, project_lead, project_with_lead, db):
...
class TestDocApproval:
    ...
    @patch('documents.services.approval_service.ApprovalService.approve_stage_one')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_doc_approval_stage_one(self, mock_get, mock_approve, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('actions', 'approve'), data, format='json')
    ...
    @patch('documents.services.approval_service.ApprovalService.approve_stage_two')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_doc_approval_stage_two(self, mock_get, mock_approve, api_client, user, project_document, db):
    ...
    @patch('documents.services.approval_service.ApprovalService.approve_stage_three')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_doc_approval_stage_three(self, mock_get, mock_approve, api_client, user, project_document, db):
    ...
    def test_doc_approval_stage_not_integer(self, api_client, user, project_document, db):
        """Test approval with non-integer stage"""
        # Arrange
        api_client.force_authenticate(user=user)
        data = {
            'stage': 'invalid',  # Not an integer
            'documentPk': project_document.id
        }
...
class TestDocRecall:
    ...
    @patch('documents.services.approval_service.ApprovalService.recall')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_doc_recall_with_reason(self, mock_get, mock_recall, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('actions', 'recall'), data, format='json')
        
        # Assert
        assert response.status_code == status.HTTP_202_ACCEPTED
        mock_recall.assert_called_once_with(project_document, user, 'Need to make changes')
        assert 'id' in response.data

    @patch('documents.services.approval_service.ApprovalService.recall')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_doc_recall_without_reason(self, mock_get, mock_recall, api_client, user, project_document, db):
...
class TestDocSendBack:
    ...
    @patch('documents.services.approval_service.ApprovalService.send_back')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_doc_send_back_with_reason(self, mock_get, mock_send_back, api_client, user, project_document, db):
        ...
        # Act
        response = api_client.post(documents_urls.path('actions', 'send_back'), data, format='json')
        
        # Assert
        assert response.status_code == status.HTTP_202_ACCEPTED
        mock_send_back.assert_called_once_with(project_document, user, 'Needs more detail')
        assert 'id' in response.data

    @patch('documents.services.approval_service.ApprovalService.send_back')
    @patch('documents.services.document_service.DocumentService.get_document')
    def test_doc_send_back_without_reason(self, mock_get, mock_send_back, api_client, user, project_document, db):
