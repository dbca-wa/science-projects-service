...
class ApprovalService:
    ...
    def request_approval(document, requester):
        ...
        settings.LOGGER.info(f"{requester} is requesting approval for document {document}")
    ...
    def approve_stage_three(document, approver):
        ...
        settings.LOGGER.info(f"{approver} is approving document {document} at stage 3 (final)")
    ...
    def _can_approve_stage_one(document, user):
        ...
        return document.project.members.filter(
            user=user,
            is_leader=True
        ).exists()
