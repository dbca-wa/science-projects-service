...
def validate_document_kind(kind):
    ...
    if kind not in valid_kinds:
        raise ValidationError(f"Invalid document kind: {kind}. Must be one of {valid_kinds}")
...
def validate_annual_report_year(year):
    ...
    if year > current_year + 1:
        raise ValidationError(f"Report year cannot be more than one year in the future")
