...
class CaretakerRequestService:
    ...
    def approve_request(task_id, approver):
        ...
        if not caretaker_id:
            raise ValidationError(
                "Cannot approve request: no caretaker specified in secondary_users"
            )

        if not (approver.is_superuser or approver.pk == caretaker_id):
            raise PermissionDenied(
                "Only the requested caretaker or an admin can approve this request"
            )
    ...
    def reject_request(task_id, rejector):
        ...
        if not (rejector.is_superuser or rejector.pk == caretaker_id):
            raise PermissionDenied(
                "Only the requested caretaker or an admin can reject this request"
            )
        ...
        settings.LOGGER.info(f"{rejector} rejected caretaker request {task_id}")
    ...
    def cancel_request(task_id, canceller):
        ...
        settings.LOGGER.info(f"{canceller} cancelled caretaker request {task_id}")
