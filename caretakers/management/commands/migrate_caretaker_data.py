...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if source_count == 0:
            self.stdout.write(self.style.SUCCESS(
                '✓ No data to migrate (source table empty)'
            ))
            return
        
        if dry_run:
            self.stdout.write(self.style.WARNING(
                f'\nWould migrate {source_count} records'
            ))
            self.stdout.write(self.style.WARNING(
        ...
        try:
            with transaction.atomic():
                ...
                if dest_count_after < source_count:
                    self.stdout.write(self.style.WARNING(
                        f'⚠ Warning: Destination has fewer records than source'
                    ))
                    self.stdout.write(self.style.WARNING(
                        f'  This may be expected if duplicates were merged'
                    ))
