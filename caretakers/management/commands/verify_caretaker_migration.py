...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for old_caretaker in OldCaretaker.objects.all():
            try:
                ...
                if new_caretaker.reason != old_caretaker.reason:
                    mismatches.append(
                        f"reason: '{old_caretaker.reason}' != '{new_caretaker.reason}'"
                    )
                if new_caretaker.notes != old_caretaker.notes:
                    mismatches.append(
                        f"notes: '{old_caretaker.notes}' != '{new_caretaker.notes}'"
                    )
                if new_caretaker.end_date != old_caretaker.end_date:
                    mismatches.append(
                        f"end_date: {old_caretaker.end_date} != {new_caretaker.end_date}"
                    )
        ...
        if not missing_in_new and not data_mismatches and not extra_in_new:
            ...
            self.stdout.write(
                self.style.SUCCESS(f"✓ All {old_count} records match perfectly")
            )
            return
        ...
        if missing_in_new:
            ...
            for caretaker in missing_in_new[:5]:  # Show first 5
                self.stdout.write(
                    f"  - ID {caretaker.id}: {caretaker.user} -> {caretaker.caretaker}"
                )
        ...
        if fix_mode and has_issues:
            ...
            self.stdout.write(
                "Example: python manage.py verify_caretaker_migration --fix"
            )
