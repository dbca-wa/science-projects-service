...
class TestCaretakerTaskService:
    ...
    def test_get_all_caretaker_assignments_direct(self, caretaker_user, caretakee_user, caretaker_assignment):
        """Test get_all_caretaker_assignments returns direct assignments"""
        # Act
        assignments = CaretakerTaskService.get_all_caretaker_assignments(caretaker_user.id)
    ...
    def test_get_directorate_documents(self, db):
        ...
        doc3 = ProjectDocument.objects.create(
            project=project1,
            kind='projectplan',
            status=ProjectDocument.StatusChoices.INAPPROVAL,
            project_lead_approval_granted=True,
    ...
    def test_analyze_caretakee_roles_directorate_by_ba(self, db, directorate_user):
        ...
        # Arrange
        caretaker = UserFactory()
        assignment = Caretaker.objects.create(user=directorate_user, caretaker=caretaker)
    ...
    @pytest.mark.django_db
    def test_get_tasks_for_user_requesting_user_is_directorate(self, db, directorate_user):
    ...
    def test_get_tasks_for_user_multiple_roles(self, db):
        ...
        ba_doc = ProjectDocument.objects.create(
            project=project,
            kind='concept',
            status=ProjectDocument.StatusChoices.INAPPROVAL,
            project_lead_approval_granted=True,
...
class TestCaretakerService:
    ...
    @pytest.mark.django_db
    def test_list_caretakers(self, caretaker_user, caretakee_user, caretaker_assignment):
    ...
    @pytest.mark.django_db
    def test_list_caretakers_optimized_queries(self, caretaker_user, caretakee_user, caretaker_assignment):
    ...
    @pytest.mark.django_db
    def test_create_caretaker_duplicate(self, caretaker_user, caretakee_user, caretaker_assignment):
        """Test create_caretaker raises ValidationError for duplicate relationship"""
        # Act & Assert
        with pytest.raises(ValidationError, match="Caretaker relationship already exists"):
            CaretakerService.create_caretaker(
                user=caretakee_user,
                caretaker=caretaker_user,
                reason='Test reason',
    ...
    def test_create_caretaker_caretaker_has_caretaker(self, db):
        ...
        with pytest.raises(ValidationError, match="Cannot set a user with a caretaker as caretaker"):
            CaretakerService.create_caretaker(
                user=user3,
                caretaker=user2,
                reason='Test reason',
    ...
    def test_update_caretaker_not_found(self, db):
        ...
        with pytest.raises(NotFound):
            CaretakerService.update_caretaker(
                pk=99999,
    ...
    @pytest.mark.django_db
    def test_get_user_caretaker_exists(self, caretaker_user, caretakee_user, caretaker_assignment):
...
class TestCaretakerRequestService:
    ...
    def test_validate_caretaker_request_wrong_action(self, db):
        ...
        # Act & Assert
        with pytest.raises(ValidationError, match="This endpoint only handles caretaker requests"):
            CaretakerRequestService.validate_caretaker_request(task, user)
    ...
    def test_validate_caretaker_request_already_processed(self, db):
        ...
        # Act & Assert
        with pytest.raises(ValidationError, match="This request has already been processed"):
            CaretakerRequestService.validate_caretaker_request(task, user)
    ...
    def test_validate_caretaker_request_unauthorized_user(self, db):
        ...
        # Act & Assert
        with pytest.raises(PermissionDenied, match="You are not authorized to respond to this request"):
    ...
    def test_approve_request_validation_error(self, db):
        ...
        with patch('caretakers.services.request_service.Caretaker.objects.create') as mock_create:
            mock_create.side_effect = Exception('Database error')
            
            # Act & Assert
            with pytest.raises(ValidationError, match="Failed to create caretaker relationship"):
