...
class TestAdminControlsDetail:
    ...
    def test_put_admin_controls_merges_guide_content(
        ...
        # Assert
        assert response.status_code == status.HTTP_202_ACCEPTED
        assert response.data["guide_content"] == {
            "existing": "content",
            "new": "content",
        }
...
class TestGuideSectionViewSet:
    ...
    def test_reorder_sections(self, api_client, admin_user, db):
        ...
        section1 = GuideSection.objects.create(
            id="section-1", title="Section 1", order=0, category="general"
        )
        section2 = GuideSection.objects.create(
            id="section-2", title="Section 2", order=1, category="general"
        )
        ...
        # Act
        response = api_client.post(
            adminoptions_urls.path("guide-sections", "reorder"), data, format="json"
        )
    ...
    def test_reorder_fields(
        ...
        field2 = ContentField.objects.create(
            section=guide_section, title="Field 2", field_key="field_2", order=1
        )
        ...
        # Act
        response = api_client.post(
            adminoptions_urls.path(
                "guide-sections", guide_section.id, "reorder_fields"
            ),
            data,
            format="json",
        )
...
class TestCheckPendingCaretakerRequestForUser:
    ...
    def test_check_pending_caretaker_request_exists(
        ...
        response = api_client.post(
            adminoptions_urls.path(
                "tasks", "check", admin_task_set_caretaker.primary_user.id
            )
        )
...
class TestGetPendingCaretakerRequestsForUser:
    ...
    def test_get_pending_caretaker_requests(self, api_client, user, secondary_user, db):
        ...
        # Act
        response = api_client.get(
            f"{adminoptions_urls.path('tasks', 'pending')}?user_id={user.id}"
        )
