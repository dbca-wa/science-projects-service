...
class TestTinyAreaSerializer:
    ...
    def test_serialize_area_fields_only(self, dbca_district, db):
        ...
        # Should not have old_id, created_at, updated_at
        assert 'old_id' not in data
...
class TestAreaSerializer:
    ...
    def test_serialize_area_values(self, dbca_district, db):
        ...
        assert data['name'] == dbca_district.name
        assert data['area_type'] == dbca_district.area_type
        assert data['old_id'] == dbca_district.old_id
    ...
    def test_deserialize_missing_old_id(self, db):
        """Test deserializing with missing old_id fails validation"""
        data = {
            'name': 'Perth District',
            'area_type': Area.AreaTypeChoices.AREA_TYPE_DBCA_DISTRICT,
        }
        ...
        assert not serializer.is_valid()
        assert 'old_id' in serializer.errors
    ...
    def test_update_area_old_id(self, dbca_district, db):
        """Test updating old_id via serializer"""
        data = {
            'old_id': 999,
        }
        serializer = AreaSerializer(dbca_district, data=data, partial=True)
    ...
    def test_update_area_full(self, dbca_district, db):
        ...
        assert area.old_id == 888

    def test_serialize_multiple_areas(self, dbca_district, dbca_region, ibra_region, db):
        """Test serializing multiple areas"""
    ...
    def test_deserialize_old_id_as_string(self, db):
        """Test deserializing old_id as string converts to int"""
        data = {
            'name': 'Test Area',
            'area_type': Area.AreaTypeChoices.AREA_TYPE_DBCA_DISTRICT,
            'old_id': '100',  # String instead of int
        }
        serializer = AreaSerializer(data=data)
    ...
    def test_deserialize_old_id_invalid_string(self, db):
        """Test deserializing old_id with invalid string fails"""
        data = {
            'name': 'Test Area',
            'area_type': Area.AreaTypeChoices.AREA_TYPE_DBCA_DISTRICT,
            'old_id': 'not_a_number',
