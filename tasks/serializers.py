...
class TinyTaskMiniUserSerializer(ModelSerializer):

    class Meta:
        model = User
        fields = ["pk", "first_name", "last_name",]

class TinyTaskMiniProjectSerializer(ModelSerializer):

    class Meta:
        model = Project
        fields = ["pk", "title", "kind",]

class TinyTaskMiniDocumentSerializer(ModelSerializer):

    project = TinyTaskMiniProjectSerializer(read_only=True)
...
class TinyTaskSerializer(ModelSerializer):
    creator = TinyTaskMiniUserSerializer(read_only=True)
    user = TinyTaskMiniUserSerializer(read_only=True)
    document = TinyTaskMiniDocumentSerializer(read_only=True)
