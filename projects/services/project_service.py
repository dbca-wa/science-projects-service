...
class ProjectService:
    ...
    def list_projects(user, filters=None):
        ...
        projects = projects.annotate(
            custom_ordering=Case(
                When(
                    status__in=["suspended", "completed", "terminated"], 
                    then=Value(1)
                ),
                default=Value(0),
    ...
    def get_project_years():
        ...
        return Project.objects.values_list('year', flat=True).distinct().order_by('-year')
    ...
    def toggle_user_profile_visibility(pk, user):
        ...
        if user.pk in project.hidden_from_staff_profiles:
            ...
            settings.LOGGER.info(f"{user} is showing project {project} on their profile")
        else:
            project.hidden_from_staff_profiles.append(user.pk)
            settings.LOGGER.info(f"{user} is hiding project {project} from their profile")
