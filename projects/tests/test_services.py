...
class TestMemberService:
    ...
    def test_get_project_leader_multiple_leaders(self, project_with_lead, user_factory, db):
    ...
    def test_add_duplicate_member_raises_error(self, project_with_lead, project_lead, superuser, db):
        ...
        with pytest.raises(ValidationError, match='already a member'):
            MemberService.add_member(project_with_lead.pk, project_lead.pk, data, superuser)
    ...
    def test_update_member(self, project_with_lead, project_lead, superuser, db):
        ...
        # Act
        updated = MemberService.update_member(project_with_lead.pk, project_lead.pk, data, superuser)
        
        # Assert
        assert updated.role == 'research'
        assert updated.time_allocation == 75

    def test_update_member_with_none_values(self, project_with_lead, project_lead, superuser, db):
        """Test updating member with None values doesn't change fields"""
        # Arrange
        original_role = ProjectMember.objects.get(project=project_with_lead, user=project_lead).role
        data = {
    ...
    def test_promote_to_leader(self, project_with_members, user_factory, superuser, db):
        ...
        # Act
        promoted = MemberService.promote_to_leader(project_with_members.pk, new_leader.pk, superuser)
...
class TestExportService:
    ...
    def test_export_all_projects_csv_with_business_area_leader(self, project_with_members, user, business_area, db):
    ...
    def test_export_all_projects_csv_with_team_members(self, project_with_members, user, db):
        ...
        for member in project_with_members.members.all():
            if member.user:
                assert member.user.first_name in content or member.user.last_name in content

    @patch('projects.services.export_service.AnnualReport')
    def test_export_annual_report_projects_csv_no_reports(self, mock_annual_report, user, db):
    ...
    @patch('projects.services.export_service.Project.objects')
    @patch('projects.services.export_service.AnnualReport')
    def test_export_annual_report_projects_csv_error(self, mock_annual_report, mock_projects, user, db):
...
class TestAreaService:
    ...
    def test_get_project_area_not_found(self, business_area, db):
        ...
        # Act & Assert
        with pytest.raises(NotFound, match=f'Project area not found for project {project_without_area.pk}'):
    ...
    def test_update_project_area(self, project_with_area, user, db):
        ...
        # Act
        updated = AreaService.update_project_area(project_with_area.pk, new_area_ids, user)
...
class TestExportServiceAdditional:
    ...
    def test_export_annual_report_projects_csv_success(self, project_with_members, user, db):
    ...
    def test_export_annual_report_with_student_report(self, project_with_members, user, db):
    ...
    def test_export_annual_report_with_both_reports(self, project_with_members, user, db):
...
class TestProjectServiceAdditional:
    ...
    def test_list_projects_with_selected_user_filter(self, user, project_with_lead, project_lead, db):
    ...
    def test_list_projects_with_multiple_business_areas(self, user, project, business_area, db):
...
class TestDetailsServiceAdditional:
    ...
    def test_update_student_details(self, project, user, db):
        ...
        details = DetailsService.create_student_details(
            project.pk,
            {'level': 'undergraduate', 'organisation': 'Test Uni'},
            user
        )
    ...
    def test_update_student_details_not_found(self, project, user, db):
        ...
        # Act & Assert
        with pytest.raises(NotFound, match=f'Student details not found for project {project.pk}'):
    ...
    def test_update_external_details(self, project, user, db):
        ...
        details = DetailsService.create_external_details(
            project.pk,
            {'collaboration_with': '<p>Original</p>'},
            user
        )
    ...
    def test_update_external_details_not_found(self, project, user, db):
        ...
        # Act & Assert
        with pytest.raises(NotFound, match=f'External details not found for project {project.pk}'):
    ...
    def test_create_project_details_with_all_fields(self, project, user, user_factory, db):
        ...
        service = DepartmentalService.objects.create(name='Test Service', old_id=1)
